...
export class AuthService {
  ...
  async authenticateWithEmailAndPassword(
    ...
    const newUser = await this.userRepository.findByUserId(
      exUser.id,
    );
  ...
  async registerWithEmail(
    ...
    // 이메일 인증 여부 확인
    const emailConfirm =
      await this.emailConfirmRepository.existsByEmail(email);
  ...
  async confirmEmail(confirmemail: ConfirmEmailDto, queryRunner: QueryRunner) {
    ...
    // 이메일 인증 번호 확인
    const emailCheck = await this.emailVerificationRepository.findByEmailAndAuthCode(
      email,
      authCode,
    );
    ...
    // 인증 완료 시 인증 번호 삭제
    await this.emailVerificationRepository.deleteById(emailCheck.id, queryRunner);
